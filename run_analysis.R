...
all_data_mean_sd <- all_data[,grepl("mean|std|Subject|ActivityId", 
                                         names(all_data))]
...
## create tidy dataset
all_data_tidy_ish = ddply(all_data_mean_sd, c("Subject","Activity"), numcolwise(mean))

## remove unwanted columns containing mean frequancy values - failed to figure 
##out why this bit of code didn't work in a more appropriate part of the function 
all_data_tidy <- all_data_tidy_ish[,!grepl(".MeanFreq", names(all_data_tidy_ish))]
